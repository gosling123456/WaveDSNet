...
class GatedFusionModule(nn.Module):
    ...
    def forward(self, feat_a, feat_b):
        ...
        self.last_W1 = w_a.detach()
        self.last_W2 = w_b.detach()
...
class WaveDSNet2(nn.Module):
    def __init__(self, opt):
        ...
        # CSDI分支
        self.bca = BidirectionalCrossAttention_window_gate(
            dim=self.inplanes,
            num_heads=4,
            dropout=0.1
        )
        ...
        # =========================================================
        # Joint edge-mask prediction head
        # 输入: cat(out1, out2, out4)，通道数为 self.inplanes * 3
        # 输出: 4通道
        #   0:2 -> block logits
        #   2:4 -> edge logits
        # =========================================================
        self.joint_head = nn.Sequential(
            Conv3Relu(self.inplanes * 3, 16),
            nn.Dropout(0.2),
    ...
    def forward(self, xa, xb, names=None):
        ...
        # out1: fusion-based change feature
        out1 = change

        # out2: explicit difference feature
        out2 = self.changediff(ms_feats)

        # out4: cross-attention enhanced feature
        out4 = self.bca(out1, out2)

        # =========================================================
        # Joint decoding
        # 不再只用 out4，而是融合 out1/out2/out4
        # 这样 block 和 edge 都能利用：
        #   1) 原始融合变化特征 out1
        #   2) 显式差分变化特征 out2
        #   3) 交互增强变化特征 out4
        # =========================================================
        joint_feat = torch.cat([out1, out2, out4], dim=1)

        # 先上采样特征，再预测 logits，避免低分辨率 logits 被直接插值导致边界变糊
        joint_feat = F.interpolate(
            joint_feat,
        ...
        # 前两个通道作为变化区域预测
        out5 = joint_logits[:, 0:2, :, :]

        # 后两个通道作为边界预测
        out6 = joint_logits[:, 2:4, :, :]
    ...
    def _init_weight(self, pretrain=''):
        ...
        if pretrain.endswith('.pth'):
            ...
            # 只加载 key 相同且 shape 相同的权重，避免 joint_head 尺寸不一致报错
            pretrained_dict = {
            ...
            print("=> ChangeDetection load {}/{} items from: {}".format(
                len(pretrained_dict),
                len(model_dict),
                pretrain
            ))
...
class WaveDSNet_Ablation(nn.Module):
    def __init__(self, opt):
        super().__init__()
        self.inplanes = int(opt.backbone.split("_")[-1]) if opt.backbone.startswith('cswin') else 64
        self.scale = ScaleInOutput(opt.input_size)
    ...
    def forward(self, xa, xb, names=None):
        ...
        ms_feats = fa1, fa2, fa3, fa4, fb1, fb2, fb3, fb4  
        change4, change = self.neck(ms_feats) # Baseline, 直接解码这个
