...
from util.save_w1w2 import save_all_gated_weights
...
def eval(opt):
    ...
    gpu_info()  
    save_path, result_save_path = check_eval_dirs(opt.resume)
    ...
    p, r, f1, miou, oa, avg_loss = test_for_metric(model, test_loader, tta=opt.tta, save_path=save_path)
...
def test_for_metric(model, eval_loader, criterion=None, tta=False, input_size=512, dual_label=False, save_path=None):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    avg_loss = 0
    val_loss = torch.tensor([0])
    scale = ScaleInOutput(input_size)
    ...
    with torch.no_grad():
        eval_tbar = tqdm(eval_loader)
        for i, (batch_img1, batch_img2, batch_label1, batch_label2, names) in enumerate(eval_tbar):
            ...
            outs = model(batch_img1, batch_img2)
            # save_all_gated_weights(
            #     model=model,
            #     names=names,
            #     save_root=save_path.replace('test', 'bca_gated_fusion'),
            #     target_size=batch_img1.shape[-2:]
            # )
            if not isinstance(outs, tuple): # 仅变化图
                outs = (outs, outs)
                val_loss = criterion(outs, labels)
            ...
            all_labels2.append(batch_label2.cpu().numpy())
            if save_path:
                batch_size = cd_pred1.shape[0]
                pred_np = cd_pred1.detach().cpu().numpy().astype(np.uint8) * 255
                for b in range(batch_size):
                    ...
                    mask = Image.fromarray(pred_np[b])
                    # input(f"{save_path}/{img_name}")
                    mask.save(f"{save_path}/{img_name}")
