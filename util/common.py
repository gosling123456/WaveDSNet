...
def check_dirs(save_path=None):
    ...
    os.makedirs(every_ckp_save_path, exist_ok=True)
    os.makedirs(best_ckp_save_path, exist_ok=True)
...
def compute_p_r_f1_miou_oa(tn_fp_fn_tps):   # 计算各种指标
    ...
    for tn_fp_fn_tp in tn_fp_fn_tps:
        ...
        p_tmp = tp / (tp + fp + np.finfo(np.float32).eps)
        r_tmp = tp / (tp + fn + np.finfo(np.float32).eps)
        f1_tmp = 2 * p_tmp * r_tmp / (r_tmp + p_tmp + np.finfo(np.float32).eps)
        iou_0_tmp = tn / (tn + fp + fn + np.finfo(np.float32).eps)
        iou_1_tmp = tp / (tp + fp + fn + np.finfo(np.float32).eps)
        miou_tmp = 0.5 * tp / (tp + fp + fn) + 0.5 * tn / (tn + fp + fn + np.finfo(np.float32).eps)
        oa_tmp = (tp + tn) / (tp + tn + fp + fn + np.finfo(np.float32).eps)
        p0 = oa_tmp
        pe = ((tp+fp)*(tp+fn)+(fp+tn)*(fn+tn))/((tp+fp+tn+fn)**2 + np.finfo(np.float32).eps)
        kappa_tmp = (p0-pe) / (1-pe + np.finfo(np.float32).eps)
        # print(p_tmp)
        # print(r_tmp)
...
def get_metrics_offline(gt_dir, pred_dir, dual_label=False):
    """
    根据两个文件夹中的图像，离线计算指标，
...
class SaveResult:
    ...
    def prepare(self):
        # 为写日志做准备，先创建这个文件
        if not os.path.isfile(self.result_save_path):
...
class CosOneCycle:  # 自己定义的策略没有state_dict,需要看源码是怎么做的
    def __init__(self, optimizer, max_lr, epochs, min_lr=None, up_rate=0.2):  # max=0.0035, min=0.00035
    ...
    def load_state_dict(self, state_dict):
        self.step_i = state_dict['step_i']
        self.max_lr = state_dict['max_lr']
        self.min_lr = state_dict['min_lr']
        self.final_lr = state_dict['final_lr']
        self.new_lr = state_dict['new_lr']
        self.epochs = state_dict['epochs']
        self.up_rate = state_dict['up_rate']
