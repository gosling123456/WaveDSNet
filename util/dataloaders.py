...
class CDDloader(data.Dataset):
    ...
    def __init__(self, opt, phase, aug=False):
        ...
        self.aug = aug
        names = [i.strip() for i in open(f'{self.data_dir}/list/{self.phase}.txt','r',encoding='utf-8').readlines()]
        self.names = []
        for name in names:
            if is_img(name):  
    ...
    def __getitem__(self, index):
        ...
        label2 = Image.open(os.path.join(self.data_dir, 'edge', label_name)).convert('P')
